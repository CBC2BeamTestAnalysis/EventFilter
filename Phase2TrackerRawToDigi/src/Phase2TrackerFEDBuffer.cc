...
//            std::cout<<"Etat du nouveau CBC :"<<((cbc_status>>i)&0x1)<<endl;
            if((cbc_status>>i)&0x1)
            {
...
//	      std::cout<<"New CBC. Added "<<(STRIPS_PADDING+STRIPS_PER_CBC)/8<<" Bytes to data"<<endl;
            }
            else
            {
    ...
    if(trackerHeader_.getConditionData())
    {
      ...
      if (bufferDiff <= 0)
      {
        ...
        ss << "payload=" << payloadSize << ",  tracker header size=" <<trackerHeader_.getTrackerHeaderSize()  << ", offset begin.="<<offsetBeginningOfChannel<<"\n";
    ...
    else
    {
      ...
      if (bufferDiff != 0)
      {
        ...
        ss << "Expected Buffer Size " << bufferSize_ << " bytes" << "\n";
        ss << "Computed Buffer Size " << bufferSize_ - bufferDiff << " bytes" << "\n";
        ss << "payload=" << payloadSize << ",  tracker header size=" <<(int)trackerHeader_.getTrackerHeaderSize()  << ", offset begin.="<<offsetBeginningOfChannel<<"\n";
	std::cout<<ss.str();
        valid_ = 0;
      }
    } 
  ...
  std::map<uint32_t,uint32_t> Phase2TrackerFEDBuffer::conditionData()
  {
	  ...
	  std::cout<<ss.str();
		...
		std::cout<<"Number of condition data ="<<size<<std::endl;
	}
      }
